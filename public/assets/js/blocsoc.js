...
(function ($) {
	...
	if ($("body").hasClass("tb-transition")) {
		...
		function RevealLoad() {
			var tl_transitIn = gsap.timeline({ defaults: { duration: 1.5, ease: Expo.easeInOut }});
				 ...
				 tl_transitIn.to("#tb-header", { y: -20, autoAlpha: 0 }, 0);
		...
		function HideLoad() {
			var tl_transitOut = gsap.timeline();
				 ...
				 // tb-Header appear
				 tl_transitOut.from("#tb-header", { duration: 1.5, y: 20, autoAlpha: 0, ease: Expo.easeInOut, clearProps:"all" }, 0.6);
				 ...
				 if ($(".tb-psc-elem").length) {
				 	$(".tb-psc-elem").wrap('<div class="tb-ps-appear"></div>');
				 	tl_transitOut.from(".tb-ps-appear", { duration: 2, y: 80, autoAlpha: 0, stagger: 0.3, ease: Expo.easeOut, clearProps:"all" }, 1.4);
				 }

				 // Portfolio carousel elements appear
				 if ($(".tb-pci-title").length) {
				 	tl_transitOut.from(".tb-pci-title", { duration: 2, x: 80, autoAlpha: 0, skewX: "-10deg", ease: Expo.easeOut, clearProps:"all" }, 1.4);
				 }
				 if ($(".tb-pci-category").length) {
				 	tl_transitOut.from(".tb-pci-category", { duration: 2, x: 80, autoAlpha: 0, ease: Expo.easeOut, clearProps:"all" }, 1.5);
				 }

				 // Sliding sidebar trigger appear
				 if ($(".tb-sliding-sidebar-wrap").length) {
					 if(!isMobile) {
						 if ($(".tb-sliding-sidebar-wrap").hasClass("tb-ss-right")) {
						 	tl_transitOut.from(".tb-sliding-sidebar-trigger", { duration: 1.5, autoAlpha: 0, x: 50, ease: Expo.easeOut }, 2);
						 } else {
						 	tl_transitOut.from(".tb-sliding-sidebar-trigger", { duration: 1.5, autoAlpha: 0, x: -50, ease: Expo.easeOut }, 2);
						 }
					 } else {
					 	tl_transitOut.from(".tb-sliding-sidebar-trigger", { duration: 1.5, autoAlpha: 0, y: 20, ease: Expo.easeOut }, 2);
					 }
				 }
	...
	if ($("body").hasClass("tb-smooth-scroll")) {
		...
		if(!isMobile) {
			...
			// Move "tb-header" out of "scroll-container"
			// Expl: Since Smooth Scrollbar doesn't support element fixed position inside "scroll-container" move the "tb-header" out of it.
			// ==========================================
			if ($("#tb-header").hasClass("tb-header-fixed")) {
				$("#tb-header").prependTo( $("#body-inner"));
			}
	...
	if ($("body").not(".is-mobile").hasClass("tb-magic-cursor")) {
		if ($(window).width() > 1024) {
			...
			$(".cursor-alter, .tb-main-menu-list > li > a, .tb-main-menu-list > li > .tb-submenu-trigger > a")
			...
			$(".tb-ol-submenu-caret-wrap .magnetic-wrap").on("mouseenter", function() {
			...
			$(".cursor-close").each(function() {
				...
				$(".cursor-close a, .cursor-close button, .cursor-close .tb-btn, .cursor-close .hide-cursor")
			...
			// Page nav hover.
			$(".tb-page-nav").each(function() {
				if ($(this).find(".tb-pn-image").length) {
					$(this).find(".tb-pn-link").on("mouseenter mouseover", function() {
						$("#magic-cursor").addClass("tb-pn-hover-on");
						$(this).parent().find(".tb-pn-image").appendTo($ball);
						gsap.to($ball, { duration: 0.3, width: "20vw", height: "20vw", opacity: 1 });
						$ball.find(".tb-pn-image video").each(function() {
							$(this).get(0).play();
						}); 
					}).on("mouseleave", function() {
						$("#magic-cursor").removeClass("tb-pn-hover-on");
						$ball.find(".tb-pn-image").appendTo(this);
						gsap.to($ball, { duration: 0.3, width: $ballWidth, height: $ballHeight, opacity: $ballOpacity });
						
						$(this).parent().find('.tb-pn-image video').each(function() {
							$(this).get(0).pause();
						}); 
					});
					$(this).find(".tb-pn-link").addClass("not-hide-cursor");
				} else {
					$(this).find(".tb-pn-link").removeClass("not-hide-cursor");
				}
			});

			// Page nav title hover.
			if ($(".tb-pn-image").length) {
				$(".tb-page-nav").each(function() {
					$(this).find(".tb-pn-link").on("mouseenter mouseover", function() {
						$("#magic-cursor").addClass("tb-page-nav-hover-on");
						$(this).parent().find(".tb-pn-image").appendTo($ball);					
						gsap.to($ball, { duration: 0.3, width: "20vw", height: "20vw", opacity: 1 });
						if ($(".tb-pn-image video").length) {
							$(".tb-pn-image video").get(0).play();
						}
					}).on("mouseleave", function() {
						$("#magic-cursor").removeClass("tb-page-nav-hover-on");
						$ball.find(".tb-pn-image").appendTo(this);
						gsap.to($ball, { duration: 0.3, width: $ballWidth, height: $ballHeight, opacity: $ballOpacity });

						if ($(".tb-pn-image video").length) {
							$(".tb-pn-image video").get(0).pause();
						}
					});
				});
				$(".tb-pn-link").addClass("not-hide-cursor");
			} else {
				$(".tb-pn-link").removeClass("not-hide-cursor");
			}
			...
			// Hide on hover.
			$("a, button, .tb-btn, .tb-form-control, .tb-form-radio, .tb-form-check, .hide-cursor") // class "hide-cursor" is for global use.
			.not(".not-hide-cursor") // omit from selection (class "not-hide-cursor" is for global use).
			.not(".cursor-alter") // omit from selection
			.not(".tb-main-menu-list > li > a") // omit from selection
			.not(".tb-main-menu-list > li > .tb-submenu-trigger > a") // omit from selection
			.on("mouseenter", function() {
	...
	gsap.utils.toArray(".tb-lazy").forEach(image => {
	...
	if ($(window).width() > 1024) {
		$(".tb-submenu-trigger").parent().on("mouseenter", function() {
			var menu = $("> .tb-submenu", this);
	...
	// Main menu hover
	$(".tb-main-menu-list").on("mouseenter", function() {
		$(this).addClass("tb-mm-hover");
	}).on("mouseleave", function() {
		$(this).removeClass("tb-mm-hover");
	});

	// Submenu wrap hover
	$(".tb-submenu-wrap").on("mouseenter", function() {
		$(this).addClass("tb-submenu-open");
	}).on("mouseleave", function() {
		$(this).removeClass("tb-submenu-open");
	});
	...
	$("#tb-m-menu-toggle-btn-wrap").on("click", function() {
		$("html").toggleClass("tb-no-scroll");
		$("body").toggleClass("tb-m-menu-open");
		if ($("body").hasClass("tb-m-menu-open")) {

			// Move "tb-header" out of "scroll-container" (no effect if "tb-header-fixed" or "tb-smooth-scroll" enabled).
			// Expl: Since Smooth Scrollbar doesn't support element fixed position inside "scroll-container" move the "tb-header" out of it.
			if ($("body").hasClass("tb-smooth-scroll")) {
				$("#tb-header:not(.tb-header-fixed)").prependTo("#body-inner");
			}

			// Menu step in animations
			$("body").addClass("tb-m-menu-toggle-no-click"); // Disable toggle button click until the animations last.

			// Menu in animations
			var tl_MenuIn = gsap.timeline({
				onComplete: function() { 
					$("body").removeClass("tb-m-menu-toggle-no-click"); 
				}
			});

				 tl_MenuIn.to(".tb-main-menu", { duration: 0.4, autoAlpha: 1 });
				 tl_MenuIn.from(".tb-main-menu-content > ul > li", { duration: 0.4, y: 80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeOut, clearProps:"all" });

			// On menu link click
			$(".tb-main-menu a, .tb-logo a")
			.not('[target="_blank"]') // omit from selection
			.not('[href^="#"]') // omit from selection
			.not('[href^="mailto"]') // omit from selection
			.not('[href^="tel"]') // omit from selection
			.on('click', function() {
				...
				gsap.to(".tb-main-menu-content > ul > li", { duration: 0.4, y: -80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeIn });
			});
			...
			var tl_MenuOut = gsap.timeline({
				onComplete: function() { 
				 ...
				 tl_MenuOut.to(".tb-main-menu-content > ul > li", { duration: 0.4, y: -80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeIn });
				 tl_MenuOut.to(".tb-main-menu", { duration: 0.4, autoAlpha: 0, clearProps:"all" }, "+=0.2");
				 tl_MenuOut.to(".tb-main-menu-content > ul > li", { clearProps:"all" }); // clearProps only

			// Close open submenus
			setTimeout(function () {
				$(".tb-submenu").slideUp(350);
				$(".tb-submenu-trigger").removeClass("tb-m-submenu-open");
			}, 500);
		}
	...
	// Append element if link href contains #
 	$('.tb-submenu-trigger > a[href^="#"]').parent(".tb-submenu-trigger").append('<span class="tb-submenu-trigger-m"></span>');

	// Open submenu if link href contains #
	if ($(".tb-submenu-trigger > a").is('[href^="#"]')) {
		$(".tb-submenu-trigger-m").on("click", function() {
			var $this = $(this).parent();
			if ($this.hasClass("tb-m-submenu-open")) {
				$this.removeClass("tb-m-submenu-open");
				$this.next().slideUp(350);
			} else {
				$this.parent().parent().find(".tb-submenu").prev().removeClass("tb-m-submenu-open");
				$this.parent().parent().find(".tb-submenu").slideUp(350);
				$this.toggleClass("tb-m-submenu-open");
	...
	$(".tb-submenu-trigger").append('<span class="tb-m-caret"></span>');
	$(".tb-m-caret").on("click", function() {
		var $this = $(this).parent();
		if ($this.hasClass("tb-m-submenu-open")) {
			$this.removeClass("tb-m-submenu-open");
			$this.next().slideUp(350);
	...
	// Add class "tb-header-fixed-on" to <body> if "tb-header-fixed" enabled.
	if ($("#tb-header").hasClass("tb-header-fixed")) {
		$("body").addClass("tb-header-fixed-on");
	}

	// On menu toggle button click
	// ============================
	var $olMenuToggleBtn = $(".tb-ol-menu-toggle-btn-text, .tb-ol-menu-toggle-btn");
	
	$olMenuToggleBtn.on("click", function() {
		$("html").toggleClass("tb-no-scroll");
		$("body").toggleClass("tb-ol-menu-open");	
		if ($("body").hasClass("tb-ol-menu-open")) {
			...
			var tl_olMenuIn = gsap.timeline({
				onComplete: function() { 
				 ...
				 tl_olMenuIn.to(".tb-overlay-menu", { duration: 0.4, autoAlpha: 1 });
				 tl_olMenuIn.from(".tb-ol-menu-list > li", { duration: 0.4, y: 80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeOut, clearProps:"all" });
				 if ($(".tb-ol-menu-anim").length) {
				 	tl_olMenuIn.from($(".tb-ol-menu-anim"), { duration: 0.4, y: 60, autoAlpha: 0, stagger: 0.05, ease: Power2.easeOut, clearProps:"all" }, "-=0.4");
				 }

			// On menu link click
			$(".tb-overlay-menu a, .tb-logo a")
			.not('[target="_blank"]') // omit from selection
			.not('[href^="#"]') // omit from selection
			.not('[href^="mailto"]') // omit from selection
			.not('[href^="tel"]') // omit from selection
			.on('click', function() {
				...
				var tl_olMenuClick = gsap.timeline();
					 tl_olMenuClick.to(".tb-ol-menu-list > li", { duration: 0.4, y: -80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeIn });
					 if ($(".tb-ol-menu-anim").length) {
					 	tl_olMenuClick.to(".tb-ol-menu-anim", { duration: 0.4, y: -60, autoAlpha: 0, stagger: 0.05, ease:Power2.easeIn }, "-=0.4");
					 }
			});

		} else {	

			// Move "tb-header" back to the "scroll-container" (in to the "scroll-content").
			if ($("body").hasClass("tb-smooth-scroll")) {
				$("#tb-header:not(.tb-header-fixed)").prependTo(".scroll-content");
			}
			...
			var tl_olMenuOut = gsap.timeline({
				onComplete: function() { 
				 ...
				 tl_olMenuOut.to(".tb-ol-menu-list > li", { duration: 0.4, y: -80, autoAlpha: 0, stagger: 0.05, ease: Power2.easeIn });
				 if ($(".tb-ol-menu-anim").length) {
				 	tl_olMenuOut.to(".tb-ol-menu-anim", { duration: 0.4, y: -60, autoAlpha: 0, stagger: 0.05, ease:Power2.easeIn }, "-=0.4");
				 }
				 tl_olMenuOut.to(".tb-overlay-menu", { duration: 0.4, autoAlpha: 0, clearProps:"all" }, "+=0.2");
				 tl_olMenuOut.set(".tb-ol-menu-list > li, .tb-ol-menu-anim", { clearProps:"all" }); // clearProps only

			// Close open submenus
			setTimeout(function () {
				$(".tb-ol-submenu").slideUp(350);
				$(".tb-ol-submenu-trigger").removeClass("tb-ol-submenu-open");
			}, 500);
		}
	...
	// Menu list hover
	$(".tb-ol-menu-list").on("mouseenter", function() {
		$(this).addClass("tb-ol-menu-hover");
	}).on("mouseleave", function() {
		$(this).removeClass("tb-ol-menu-hover");
	});

	// Open submenu if link href contains #
	$(".tb-ol-submenu-trigger > a").on("click", function() {
		if ($(this).is('[href^="#"]')) {
			var $this = $(this).parent();
			if ($this.hasClass("tb-ol-submenu-open")) {
				$this.removeClass("tb-ol-submenu-open");
				$this.next().slideUp(350);
			} else {
				$this.parent().parent().find(".tb-ol-submenu").prev().removeClass("tb-ol-submenu-open");
				$this.parent().parent().find(".tb-ol-submenu").slideUp(350);
				$this.toggleClass("tb-ol-submenu-open");
				$this.next().slideToggle(350);
			}
		}
		return false;
	...
	// Open submenu on caret click
	$(".tb-ol-submenu-caret-wrap").on("click", function() {
		var $this = $(this).parent();
		if ($this.hasClass("tb-ol-submenu-open")) {
			$this.removeClass("tb-ol-submenu-open");
			$this.next().slideUp(350);
	...
	if ($(".tb-portfolio-slider").length) {
		$(".tb-portfolio-slider").each(function() {
			...
			var $ttPortfolioSliderSwiper = new Swiper ($ttPortfolioSlider.find(".swiper-container")[0], {
				...
				// Pagination
				pagination: {
					el: ".tb-ps-nav-pagination",
					type: $dataPaginationType,
					modifierClass: "tb-ps-nav-pagination-",
					dynamicBullets: true,
				...
				// Navigation arrows
				navigation: {
					nextEl: $(this).find(".tb-ps-nav-arrow-next")[0],
					prevEl: $(this).find(".tb-ps-nav-arrow-prev")[0],
					disabledClass: "tb-ps-nav-arrow-disabled",
				},
				...
				on: {
					init: function () {
						...
						if ($ttPortfolioSlider.find(".tb-ps-caption-title").find("a").length) {
							$ttPortfolioSlider.find(".tb-ps-caption-title a").text($slideActive.attr("data-title"));
							$ttPortfolioSlider.find(".tb-ps-caption-title a").attr("href", $slideActive.attr("data-url"));
						} else {
							$ttPortfolioSlider.find(".tb-ps-caption-title").text($slideActive.attr("data-title"));
						}

						// Portfolio slider caption category on load
						$ttPortfolioSlider.find(".tb-ps-caption-category").text($slideActive.attr("data-category"));
					},
					...
					transitionStart: function () {
						...
						// If slider image is dark
						setTimeout(function(){
							if ($slideActive.hasClass("psi-image-is-dark")) {
								$("body").addClass("psi-dark-image-on");
							} else {
								$("body").removeClass("psi-dark-image-on");
							}
						}, 400);
						...
						gsap.fromTo($ttPortfolioSlider.find(".tb-psc-elem"), { autoAlpha: 1, y: 0 }, { duration: 0.25, autoAlpha: 0, y: -30, stagger: 0.15, ease: Power1.easeIn });
					},
					...
					transitionEnd: function () {
						...
						// Animate portfolio slider caption
						gsap.fromTo($ttPortfolioSlider.find(".tb-psc-elem"), { autoAlpha: 0, y: 30 }, { duration: 0.25, autoAlpha: 1, y: 0, stagger: 0.15, ease: Power1.easeOut });
					}
				}
			});
			...
			if(!isMobile) {
				if ($ttPortfolioSlider.data("parallax-mouse-move")) {
					gsap.set($ttPortfolioSlider.find(".tb-psi-image"), { scale: 1.05 });

					$ttPortfolioSlider.mousemove(function(e) {
						parallaxIt(e, $ttPortfolioSlider.find(".tb-psi-image"), -25); // Parallax element
						parallaxIt(e, $ttPortfolioSlider.find(".tb-ps-caption-inner"), -35); // Parallax element
					});
	...
	if ($(".tb-portfolio-carousel").length) {
		$(".tb-portfolio-carousel").each(function() {
			...
			var $ttPortfolioCarouselSwiper = new Swiper ($ttPortfolioCarousel.find(".swiper-container")[0], {
				...
				pagination: {
					el: ".tb-pc-pagination",
					type: $dataPaginationType,
					modifierClass: "tb-pc-pagination-",
					dynamicBullets: true,
					dynamicMainBullets: 1,
					clickable: true,
				},
				...
				// Navigation arrows
				navigation: {
					nextEl: ".tb-pc-arrow-next",
					prevEl: ".tb-pc-arrow-prev",
					disabledClass: "tb-pc-arrow-disabled",
				},
				...
				on: {
					...
					init: function () {
						...
						$slideActive.addClass("tb-slide-active"); // Add class to active slide.

						// Carousel slide disabled (prev/next slide) on load
						$slideActive.prevAll().addClass("tb-pcs-disabled");
						$slideActive.nextAll().addClass("tb-pcs-disabled");
					...
					transitionStart: function () {
						...
						$slideActive.addClass("tb-slide-active"); // Add class to active slide.
						$slideActive.prev().addClass("tb-slide-active-start"); // Add class if active slide transition starts.
						$slideActive.next().addClass("tb-slide-active-start"); // Add class if active slide transition starts.

						// Carousel slide disabled (prev/next slide)
						$slideActive.prevAll().addClass("tb-pcs-disabled");
						$slideActive.removeClass("tb-pcs-disabled");
						$slideActive.nextAll().addClass("tb-pcs-disabled");
						...
						$(".tb-pc-arrow").addClass("tb-pc-arrow-disabled");
					...
					transitionEnd: function () {
						...
						// Active slide classes (custom)
						$slideActive.prevAll().removeClass("tb-slide-active"); // Remove class if active slide transition ends.
						$slideActive.nextAll().removeClass("tb-slide-active"); // Remove class if active slide transition ends.
						$slideActive.prev().removeClass("tb-slide-active-start"); // Remove class if active slide transition ends.
						$slideActive.next().removeClass("tb-slide-active-start"); // Remove class if active slide transition ends.
						...
						// Disable nav arrow action.
						$(".tb-pc-arrow").removeClass("tb-pc-arrow-disabled");
	...
	if ($(".tb-content-carousel").length) {
		$(".tb-content-carousel").each(function() {
			...
			var $ttContentCarouselSwiper = new Swiper($ttContentCarousel.find(".swiper-container")[0], {
				...
				// Navigation (arrows)
				navigation: {
					nextEl: $ttContentCarousel.find(".tb-cc-nav-next")[0],
					prevEl: $ttContentCarousel.find(".tb-cc-nav-prev")[0],
					disabledClass: "tb-cc-nav-arrow-disabled",
				},
				...
				pagination: {
					el: ".tb-cc-pagination",
					type: $dataPaginationType,
					modifierClass: "tb-cc-pagination-",
					dynamicBullets: true,
					dynamicMainBullets: 1,
					clickable: true,
				},
			...
			if ($ttContentCarousel.attr("data-simulate-touch") == "true") {
				if ($ttContentCarousel.hasClass("cc-scale-down")) {
					$ttContentCarousel.find(".swiper-wrapper").on("mousedown touchstart pointerdown", function(e) {
						if (e.which === 1) { // Affects the left mouse button only!
							gsap.to($ttContentCarousel.find(".tb-content-carousel-item"), { duration: 0.7, scale: 0.9 });
						}
					});
					$("body").on("mouseup touchend pointerup mouseleave", function() {	
						gsap.to($ttContentCarousel.find(".tb-content-carousel-item"), { duration: 0.7, scale: 1, clearProps: "scale" });
					});
				}
			}
	...
	if ($(".tb-testimonials-slider").length) {
		$(".tb-testimonials-slider").each(function() {
			...
			var $ttTestimonialsSliderSwiper = new Swiper ($ttTestimonialsSlider.find(".swiper-container")[0], {
				...
				// Navigation (arrows)
				navigation: {
					nextEl: $ttTestimonialsSlider.find(".tb-ts-nav-next")[0],
					prevEl: $ttTestimonialsSlider.find(".tb-ts-nav-prev")[0],
					disabledClass: "tb-ts-nav-arrow-disabled",
				},
				...
				pagination: {
					el: ".tb-ts-pagination",
					type: "bullets",
					modifierClass: "tb-ts-pagination-",
					dynamicBullets: true,
					dynamicMainBullets: 1,
					clickable: true,
				}
   ...
   // ============================================================================
	...
	// If page header background image is dark (toggle class on scroll handled by scrollTrigger plugin below)
	// =========================================
	if ($("#page-header").is(".ph-bg-image.ph-bg-image-is-dark")) {
		$("body").addClass("ph-bg-image-dark-on");
	} else {
		$("body").removeClass("ph-bg-image-dark-on");
	}
	...
	if ($("#page-header").hasClass("ph-content-parallax")) {
		...
		// If page header background image is dark
		if ($("#page-header").is(".ph-bg-image.ph-bg-image-is-dark")) {
			if ($("#tb-header").hasClass("tb-header-fixed")) {
	...
	// tb-Grid "layout-creative" parallax
	// ===================================
	ScrollTrigger.matchMedia({
		"(min-width: 768px)": function() {
			$(".tb-grid.ttgr-layout-creative-1 .tb-grid-item:nth-of-type(6n+2) .ttgr-item-inner, .tb-grid.ttgr-layout-creative-1 .tb-grid-item:nth-of-type(6n+4) .ttgr-item-inner, .tb-grid.ttgr-layout-creative-2 .tb-grid-item:nth-of-type(4n+2) .ttgr-item-inner, .tb-grid.ttgr-layout-creative-2 .tb-grid-item:nth-of-type(4n+3) .ttgr-item-inner").each(function() {
				var $this = $(this);
	...
	// Play video on hover
	$(".tb-gallery-video-wrap").on("mouseenter", function() {
		$(this).find("video").each(function() {
			$(this).get(0).play();
		}); 
	}).on("mouseleave", function() {
	...
	$(".tb-gallery-item").each(function() {
		if ($(".tb-gallery").hasClass("ttga-hover")) {
			$(this).find(".tb-gallery-image img").wrap('<div class="tb-gallery-image-hover-zoom"></div>');
		}
	});
	...
	$(".tb-accordion").each(function() {
		$(this).find(".tb-accordion-heading").on("click", function() {
			var $this = $(this);

			if ($this.parents(".tb-accordion-item").hasClass("active")) {
				$this.parents(".tb-accordion-item").removeClass("active");
				$this.next(".tb-accordion-content").slideUp(350);
			} else {
				$this.parent().parent().find(".tb-accordion-item").removeClass("active");
				$this.parent().parent().find(".tb-accordion-content").slideUp(350);
				$this.parents(".tb-accordion-item").toggleClass("active");
				$this.next(".tb-accordion-content").slideToggle(350);
			}
	...
	if(!isMobile) { // No effect on mobile devices!
		...
		if ($(".tb-page-nav").hasClass("tb-pn-scroll")) {

			$(".tb-page-nav").find(".tb-pn-hover-title").each(function() {
				var $this = $(this);
				$this.wrapInner('<span></span>');
	...
	// If tb-sidebar exist.
	if ($(".tb-sidebar").length) {

		$("body").addClass("tb-sidebar-on");

		// If tb-sidebar has class "sidebar-left" add class "sidebar-left-on" to <pody>.
		if ($(".tb-sidebar").hasClass("sidebar-left")) {
			$("body").addClass("tb-sidebar-left-on");
		}

		// If tb-sidebar has class "sidebar-right" add class "sidebar-right-on" to <pody>.
		if ($(".tb-sidebar").hasClass("sidebar-right")) {
			$("body").addClass("tb-sidebar-right-on");
		}
	}
	...
	if ($(".tb-sliding-sidebar-wrap").length) {
		...
		$("body").addClass("tb-sliding-sidebar-on, tb-sliding-sidebar-left-on");  // left position is by default.

		// Append sliding sidebar.
		$(".tb-sliding-sidebar-wrap").appendTo("#body-inner");

		// Open/close sliding sidebar.
		$(".tb-sliding-sidebar-trigger").on("click", function() {
			$("body").toggleClass("tb-sliding-sidebar-open");
		}); 
		$(".tb-sliding-sidebar-close").on("click", function() {
			$("body").removeClass("tb-sliding-sidebar-open");
		}); 

		// If sliding sidebar has class "tb-ss-right" add class "tb-ss-right-on" to <pody>.
		if ($(".tb-sliding-sidebar-wrap").hasClass("tb-ss-right")) {
			$("body").removeClass("tb-sliding-sidebar-left-on");
			$("body").addClass("tb-sliding-sidebar-right-on");
		}
	}
	...
	// Hover scrolling speed.
	$(".tb-scrolling-text").each(function() {
		var $tt_stSpeed = $(this).data("scroll-speed");
		$(this).find(".tb-scrolling-text-inner").css({ 
	...
	$(":file").on("fileselect", function(event, numFiles, label) {
		var input = $(this).parents(".tb-form-file").find(".tb-file-info"),
	...
	// E-mail Ajax Send
	$("#tb-contact-form").submit(function() { // Change (your contact form ID)
		var th = $(this);
		$.ajax({
			type: "POST",
